...
export class TaskApiService {
  ...
  async getTask(data: GetTaskQuery['data']) {
    return this.queryBus.execute<GetTaskQuery, GetTaskQueryReturnType>(new GetTaskQuery(data));
  }

  async getTasks(data: GetTasksQuery['data']) {
    return this.queryBus.execute<GetTasksQuery, GetTasksQueryReturnType>(new GetTasksQuery(data));
  }
}
